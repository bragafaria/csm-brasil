// app/components/couples/SocialConnectionsPage.js
"use client";
...
import SocialConnectionsTemplate from "@/app/lib/couple/SocialConnectionsTemplate";

export default function SocialConnectionsPage({ reportData: propReportData }) {
  ...
  useEffect(() => {
    ...
    async function fetchReportData() {
      ...
      try {
        ...
        console.log("SocialConnectionsPage session user ID:", session.user.id);
  ...
  if (!reportData?.partnerA.has_assessment || !reportData?.partnerB.has_assessment) {
    ...
    return (
      <div className="p-6 mt-20">
        <h1 className="text-3xl font-bold text-[var(--text-primary)] mb-6">Social Connections Report</h1>
  ...
  const { description } = SocialConnectionsTemplate.generateText(partnerA, partnerB);
      ...
      <div className="w-full max-w-6xl bg-[var(--surface-variant)] p-10 rounded-2xl shadow-xl border border-[var(--border)] space-y-12">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-bold text-[var(--text-primary)] tracking-tight">
            Social Connections for {partnerA.name} and {partnerB.name}
          </h1>
          <p className="text-xl italic text-[var(--text-secondary)] max-w-2xl mx-auto">
            Your social connections shape the community around your relationship. Understanding how you both approach
            friendships and social interactions can help you build a supportive network and strengthen your partnership.
          </p>
        </motion.div>
