// app/components/couples/HealthWellnessPage.js
"use client";
...
import HealthWellnessTemplate from "@/app/lib/couple/HealthWellnessTemplate";

export default function HealthWellnessPage({ reportData: propReportData }) {
  ...
  useEffect(() => {
    ...
    async function fetchReportData() {
      ...
      try {
        ...
        console.log("HealthWellnessPage session user ID:", session.user.id);
  ...
  if (!reportData?.partnerA.has_assessment || !reportData?.partnerB.has_assessment) {
    ...
    return (
      <div className="p-6 mt-20">
        <h1 className="text-3xl font-bold text-[var(--text-primary)] mb-6">Health & Wellness Report</h1>
  ...
  const { description } = HealthWellnessTemplate.generateText(partnerA, partnerB);
      ...
      <div className="w-full max-w-6xl bg-[var(--surface-variant)] p-10 rounded-2xl shadow-xl border border-[var(--border)] space-y-12">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-bold text-[var(--text-primary)] tracking-tight">
            Health & Wellness for {partnerA.name} and {partnerB.name}
          </h1>
          <p className="text-xl italic text-[var(--text-secondary)] max-w-2xl mx-auto">
            Your approach to health and wellness shapes your vitality as a couple. Understanding how you both prioritize
            physical and mental well-being can help you support each other in living healthier, more balanced lives
            together.
          </p>
        </motion.div>
