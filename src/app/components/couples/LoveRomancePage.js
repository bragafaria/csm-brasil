// app/components/couples/LoveRomancePage.js
"use client";
...
import LoveRomanceTemplate from "@/app/lib/couple/LoveRomanceTemplate";

export default function LoveRomancePage({ reportData: propReportData }) {
  ...
  useEffect(() => {
    ...
    async function fetchReportData() {
      ...
      try {
        ...
        console.log("LoveRomancePage session user ID:", session.user.id);
  ...
  if (!reportData?.partnerA.has_assessment || !reportData?.partnerB.has_assessment) {
    ...
    return (
      <div className="p-6 mt-20">
        <h1 className="text-3xl font-bold text-[var(--text-primary)] mb-6">Love & Romance Report</h1>
  ...
  const { description } = LoveRomanceTemplate.generateText(partnerA, partnerB);
      ...
      <div className="w-full max-w-6xl bg-[var(--surface-variant)] p-10 rounded-2xl shadow-xl border border-[var(--border)] space-y-12">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-bold text-[var(--text-primary)] tracking-tight">
            Love & Romance for {partnerA.name} and {partnerB.name}
          </h1>
          <p className="text-xl italic text-[var(--text-secondary)] max-w-2xl mx-auto">
            Your romantic connection is a unique dance of hearts and minds. Understanding how you both express love,
            connect emotionally, and nurture your bond can deepen your relationship and create lasting intimacy.
          </p>
        </motion.div>
