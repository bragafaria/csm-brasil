// app/components/couples/PersonalGrowthPage.js
"use client";
...
import PersonalGrowthTemplate from "@/app/lib/couple/PersonalGrowthTemplate";

export default function PersonalGrowthPage({ reportData: propReportData }) {
  ...
  useEffect(() => {
    ...
    async function fetchReportData() {
      ...
      try {
        ...
        console.log("PersonalGrowthPage session user ID:", session.user.id);
  ...
  if (!reportData?.partnerA.has_assessment || !reportData?.partnerB.has_assessment) {
    ...
    return (
      <div className="p-6 mt-20">
        <h1 className="text-3xl font-bold text-[var(--text-primary)] mb-6">Personal Growth Report</h1>
  ...
  const { description } = PersonalGrowthTemplate.generateText(partnerA, partnerB);
      ...
      <div className="w-full max-w-6xl bg-[var(--surface-variant)] p-10 rounded-2xl shadow-xl border border-[var(--border)] space-y-12">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-bold text-[var(--text-primary)] tracking-tight">
            Personal Growth for {partnerA.name} and {partnerB.name}
          </h1>
          <p className="text-xl italic text-[var(--text-secondary)] max-w-2xl mx-auto">
            Personal growth strengthens your relationship by fostering self-awareness and mutual support. Understanding
            how you both pursue self-improvement can inspire you to grow together and create a deeper, more fulfilling
            partnership.
          </p>
        </motion.div>
