// src/app/dashboard/[siteId]/communication-conflict/page.js
"use client";
...
import CommunicationConflictTemplate from "@/app/lib/couple/CommunicationConflictTemplate"; // Use alias

export default function CommunicationConflictPage() {
  ...
  useEffect(() => {
    async function fetchReportData() {
      ...
      try {
        ...
        console.log("CommunicationConflict session user ID:", session.user.id);
  ...
  if (!reportData?.partnerA.has_assessment || !reportData?.partnerB.has_assessment) {
    ...
    return (
      <div className="p-6 mt-20">
        <h1 className="text-3xl font-bold text-[var(--text-primary)] mb-6">Communication & Conflict Report</h1>
  ...
  const { description } = CommunicationConflictTemplate.generateText(partnerA, partnerB);
      ...
      <div className="w-full max-w-6xl bg-[var(--surface-variant)] p-10 rounded-2xl shadow-xl border border-[var(--border)] space-y-12">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-bold text-[var(--text-primary)] tracking-tight">
            Communication & Conflict for {partnerA.name} and {partnerB.name}
          </h1>
          <p className="text-xl italic text-[var(--text-secondary)] max-w-2xl mx-auto">
            Effective communication and conflict resolution are the backbone of a strong relationship. Understanding how
            you both express yourselves and navigate disagreements can foster deeper understanding and a more resilient
            partnership.
          </p>
        </motion.div>
