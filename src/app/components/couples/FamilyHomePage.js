// src/app/dashboard/[siteId]/family-home/page.js
"use client";
...
import FamilyHomeTemplate from "@/app/lib/couple/FamilyHomeTemplate"; // Use alias
...
export default function FamilyHomePage() {
  ...
  useEffect(() => {
    async function fetchReportData() {
      ...
      try {
        ...
        console.log("FamilyHome session user ID:", session.user.id);
  ...
  if (!reportData?.partnerA.has_assessment || !reportData?.partnerB.has_assessment) {
    ...
    return (
      <div className="p-6 mt-20">
        <h1 className="text-3xl font-bold text-[var(--text-primary)] mb-6">Family & Home Report</h1>
  ...
  const { description } = FamilyHomeTemplate.generateText(partnerA, partnerB);
      ...
      <div className="w-full max-w-6xl bg-[var(--surface-variant)] p-10 rounded-2xl shadow-xl border border-[var(--border)] space-y-12">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-bold text-[var(--text-primary)] tracking-tight">
            Family & Home for {partnerA.name} and {partnerB.name}
          </h1>
          <p className="text-xl italic text-[var(--text-secondary)] max-w-2xl mx-auto">
            Your family and home life is the heart of your shared journey. Understanding how you both manage routines,
            create a nurturing environment, and plan for your family’s future can strengthen your bond and build a
            harmonious home.
          </p>
        </motion.div>
