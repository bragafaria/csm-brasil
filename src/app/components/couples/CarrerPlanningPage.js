...
export default function CareerPlanningPage() {
  ...
  if (!reportData?.partnerA.has_assessment || !reportData?.partnerB.has_assessment) {
    ...
    return (
      <div className="p-6 mt-20">
        <h1 className="text-3xl font-bold text-[var(--text-primary)] mb-6">Career & Ambition Report</h1>
  ...
  const { description } = CareerAmbitionTemplate.generateText(partnerA, partnerB);
      ...
      <div className="w-full max-w-6xl bg-[var(--surface-variant)] p-10 rounded-2xl shadow-xl border border-[var(--border)] space-y-12">
        ...
        >
          <h1 className="text-4xl md:text-5xl font-bold text-[var(--text-primary)] tracking-tight">
            Career & Ambition for {partnerA.name} and {partnerB.name}
          </h1>
          <p className="text-xl italic text-[var(--text-secondary)] max-w-2xl mx-auto">
            Your careers are a big part of your shared journey, and understanding how you both chase your dreams can
            bring you closer. By exploring how you plan, decide, and support each other’s ambitions, you can build a
            future that feels exciting, balanced, and true to who you both are.
          </p>
        </motion.div>
