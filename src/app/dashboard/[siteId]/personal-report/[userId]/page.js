...
export default function Report() {
  ...
  useEffect(() => {
    async function fetchUserData() {
      try {
        console.log("Starting fetchUserData with params:", params);
        setLoading(true);
        setError(null);
        ...
        if (sessionError || !sessionData?.session) {
          ...
          try {
            ...
            setError("Failed to redirect to login. Please navigate to /login manually.");
          }
          return;
        }

        // Add timeout for supabase.auth.getUser()
        ...
        const {
          data: { user },
          error: authError,
        } = await Promise.race([supabase.auth.getUser(), authTimeoutPromise]);
        ...
        console.log("Validating siteId:", params.siteId, "against user.id:", user.id);
        if (!params.siteId || params.siteId !== user.id) {
          console.error("User ID mismatch:", params.siteId, user.id);
          setError("Unauthorized access to this report. Please check the URL.");
          return;
        }

        // Fetch user data from the users table with timeout
        const queryTimeoutPromise = new Promise((_, reject) => {
          setTimeout(() => reject(new Error("User data query timed out")), 5000);
        });
        ...
        const { data: userData, error: fetchError } = await Promise.race([
          supabase
            .from("users")
            .select(`"typeCode", percents, dominants, categories, has_assessment, id`)
            .eq("id", user.id)
            .single(),
          queryTimeoutPromise,
        ]);
        ...
        if (fetchError) {
          ...
          setError("Failed to load report data: " + fetchError.message);
          return;
        }

        // Validate userId from URL
        console.log("Validating userId:", params.userId, "against id:", userData.id);
        if (!params.userId || params.userId !== userData.id) {
          console.error("Stripe ID mismatch:", params.userId, userData.id);
          setError("Invalid Stripe customer ID. Please check the URL.");
          return;
        }

        if (!userData || !userData.typeCode || !userData.has_assessment) {
          console.error("No user data or incomplete assessment:", userData);
          const fallbackTypeCode = user.user_metadata?.typeCode;
          console.log("Fallback typeCode:", fallbackTypeCode);
          if (fallbackTypeCode) {
            console.log("Using fallback typeCode from user_metadata:", fallbackTypeCode);
            setData({
              percents: userData?.percents || [],
              dominants: userData?.dominants || [],
              categories: userData?.categories || [],
              typeCode: fallbackTypeCode,
              archetype: archetypes[fallbackTypeCode] || {
                name: "Unknown",
                description: "No archetype data available.",
              },
            });
          } else {
            console.error("No fallback typeCode available");
            setError("Report data not found or assessment incomplete.");
            return;
          }
        } else {
          console.log("Constructing report data:", userData);
          const reportData = {
            percents: userData.percents || [],
            dominants: userData.dominants || [],
            categories: userData.categories || [],
            typeCode: userData.typeCode,
            archetype: archetypes[userData.typeCode] || {
              name: "Unknown",
              description: "No archetype data available.",
            },
          };
        ...
        console.error("Unexpected error in fetchUserData:", err.message, err.stack);
    ...
    fetchUserData();
  }, [router, params, params.siteId, params.userId]);
  ...
  const handleCouplesReportClick = () => {
    router.push(`/report/${data?.typeCode}/couples`);
  };

  console.log("Current state - loading:", loading, "error:", error, "data:", data);

  if (loading) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[var(--surface)] text-[var(--text-primary)]">
        <div className="text-center p-8 text-xl font-medium">Loading...</div>
      </main>
    );
  }

  if (error || !data || !reportTemplates[data?.typeCode] || !archetypes[data?.typeCode]) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[var(--surface)] text-[var(--text-primary)]">
        <div className="text-center p-8 text-xl font-medium">
          {error || `Report Not Found for ${data?.typeCode || "unknown"}`}
          {error && (
            <button onClick={() => router.push("/login")} className="btn-primary mt-4 py-2 px-4 rounded-lg">
              Log In
            </button>
          )}
        </div>
  ...
  const { percents, dominants, categories, archetype, typeCode } = data;
  ...
  return (
    <motion.main
      ...
      className="flex min-h-screen flex-col mt-10 lg:mt-15 items-center py-12 px-4 bg-[var(--surface)] text-[var(--text-primary)]"
    >
